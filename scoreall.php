...
$response = str_ireplace(' ','',$response);
$response = str_ireplace('</td><tdalign="center">选修</td><tdalign="center"><palign="center">','AAA',$response);
$response = str_ireplace('</td><tdalign="center">必修</td><tdalign="center"><palign="center">','AAA',$response);
$response = str_ireplace('</td><tdalign="center">任选</td><tdalign="center"><palign="center">','AAA',$response);

preg_match_all("#[0-9]{3}[\S]{20,400}[0-9]{1,2}&nbsp;</P>#",$response,$d);
$sum=count($d[0]);
for($i=0;$i<$sum;$i++){
preg_match("#[0-9]{3}</td><tdalign=\"center\">[\S]{1,3}</td><tdalign=\"center\">([\S]{2,100})</td><tdalign=\"center\">[\S]{1,100}</td><tdalign=\"center\">([\S]{1,5})AAA([0-9]{1,3})&nbsp#",$d[0][$i],$e[$i]);
}
for($i=0;$i<$sum;$i++){
	$score=$e[$i][3];
	if($score>=90){$e[$i][3]=4;}
