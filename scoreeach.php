...
$cookie = $str[1];
$url='http://202.121.66.80/gradeLnAllAction.do?type=ln&oper=qbinfo';
$ch = curl_init($url);
...
$response=split('aname', $response);
for($tt=1;$tt<count($response);$tt++){
	preg_match_all("#[0-9]{3}</td><tdalign=\"center\">[\S]{1,3}</td><tdalign=\"center\">([\S]{2,100})</td><tdalign=\"center\">[\S]{1,100}</td><tdalign=\"center\">([\S]{1,5})AAA<palign=\"center\">([0-9]{1,3})#",$response[$tt],$d);
	$sum=count($d[0]);
	for($i=0;$i<$sum;$i++){
	preg_match("#[0-9]{3}</td><tdalign=\"center\">[\S]{1,3}</td><tdalign=\"center\">([\S]{2,100})</td><tdalign=\"center\">[\S]{1,100}</td><tdalign=\"center\">([\S]{1,5})AAA<palign=\"center\">([0-9]{1,3})#",$d[0][$i],$e[$i]);
	}
	for($i=0;$i<$sum;$i++){
		$score=$e[$i][3];
		if($score>=90){$e[$i][3]=4;}
